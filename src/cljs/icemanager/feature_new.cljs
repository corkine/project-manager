  ...
  (let [rs_check (not (string/blank? rs_id))
    ...
    {:error (if (and rs_check title_check)
              ...
              {:rs_id (if (not rs_check) "RS 编号不合法！" nil)
               :title (if (not title_check) "项目名称不能为空" nil)})
      ...
      (let [server-back @(rf/subscribe [:add-feature-server-back])]
        (modals/modal-button
          ...
          [:div {:style {:color "black"}}
           [common-fields :rs_id "RS 编号 *" "项目 ID 号，大写且唯一，仅能包含 - _ 和英文字母"]
           [common-fields :title "项目名称 *" "项目的中文名称"]
           [common-fields :description "项目概述" "简短的描述项目用途，可为空"
            {:type :textarea :attr {:rows 2}}]
           [common-fields :version "所属版本 *" "项目引入的版本"
            {:type :select :selects feature/ice-versions}]
           [common-fields :status "当前状态 *" "当前项目的开发状态，可为空"
