  ...
  (let [rand-uu #(first (string/split (random-uuid) "-"))
    ...
    (letfn [(common-field [id label hint {:keys [type selects moreNode rows readonly
      ...
      [:<>
       ...
       [common-field :title "项目名字" "简短易懂"]
       [common-field :description "描述" "对项目的描述，第一个全角句号前必须是一句话精简描述。"
        {:type :textarea :rows 2}]
       [common-field :version "项目分类" "输入项目的分类"
       ...
       [common-field [:info :apiRes] "外部测试环境 URL"
        "本项目的外部 SwaggerUI/PostMan/ApiMan 测试环境 URL"]
       [common-field [:info :feature-version] "项目版本"
        "当前项目的版本，用于标记项目和评审文档"]
       [common-field [:info :limit] "设计约束" "当前项目的设计约束"
        {:type :textarea :rows 2}]
       ...
       [:a.is-danger.mt-4.dui-tips
        {:style        {:color :red}
         :data-tooltip "点选以删除此项目，删除后关于此项目所有方案、评审等信息都将被移除"
